    ...
    product.innerHTML += `<div class="group relative flex flex-col bg-white rounded-lg shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-stone-100">
          ...
          <div class="p-6 flex-1 flex flex-col">
            <div class="flex justify-between items-start mb-2">
              <div>
                <p class="text-xs text-stone-400 uppercase tracking-widest mb-1">${card.category}</p>
                <h3 class="text-lg font-medium text-stone-900 leading-tight">${card.title}</h3>
              </div>
              <p class="text-lg font-serif font-bold text-stone-800">${card.price}</p>
            </div>
            ...
            <div class="flex items-center gap-2 mb-4 text-xs">
              <div class="flex items-center text-amber-500">
                ...
                <span class="text-stone-800 font-bold ml-1">${card.rating}</span>
                <span class="text-stone-400 font-normal ml-1">(76 reviews)</span>
              </div>
              <span class="text-stone-300">•</span>
              <span class="bg-stone-100 text-stone-600 px-2 py-0.5 rounded-full font-medium">${card.stock} in Stock</span>
            </div>
